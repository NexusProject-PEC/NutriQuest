...
class BodyFatProgressPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<DateTime> dates = progressData.map((entry) => DateTime.parse(entry['date'])).toList();
    List<double> bodyFatValues = progressData.map((entry) => (entry['bfp'] as num).toDouble()).toList();
                  ...
                  lineBarsData: [
                    LineChartBarData(
                      spots: List.generate(
                        ...
                        (index) => FlSpot(index.toDouble(), bodyFatValues[index]),
                      ),
                      isCurved: true,
                      barWidth: 3,
                      color: Colors.blue,
